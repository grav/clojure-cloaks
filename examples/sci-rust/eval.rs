...
unsafe extern "C" {
    ...
    fn eval_string(thread: i64, expression: *const c_char) -> *const c_char;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        if graal_create_isolate(ptr::null(), &mut isolate, &mut thread) != 0 {
            return Err("Could not create the SCI runtime".into());
        }
        let result = eval_string(thread as i64, expression.as_ptr());
