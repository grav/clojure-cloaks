...
| Glojure OCI service | [Working demo](examples/glojure/) | Static native build; scratch OCI image; health, named/default and Unicode requests under a non-root, read-only container; Linux ARM64. |
...
| jank native graphics | [Working demo](examples/jank/) | Native macOS and Linux ARM64 SDL windows; synthetic Space key and captured pixels verified. Linux ARM64 source build also passes jank JIT/AOT health checks; x86-64 emulation remains an optional fallback. |
| Swish + SwiftUI | [Working demo](examples/swish/) | macOS ARM64 build and script boundary tests; iOS 26.5 simulator build, install, launch and screenshot verified on the Mac host over SSH. |
...
| ClojureCLR / C# interop | [Working demo](examples/clr/) | Linux ARM64, .NET 8: C# calls ClojureCLR; .NET formatting and UTC date APIs; default, Unicode and quoted-input checks passed. |
| SCI / self-interpreting mini-Clojure | [Working demo](examples/sci-tower/) | Babashka/SCI on Linux ARM64: four nested interpreter copies; 32 assertions for greetings, recursion, closures, quoting and errors passed. |
| ClojureDart + Flutter | [Demo + screenshots](examples/clojuredart/) | Web release + Chromium, native Linux ARM64 + GTK, and iOS 26.5 simulator: Unicode/blank greetings and counter integration checks passed. |
| Scittle / browser script tags | [Demo + screenshot](examples/scittle/) | One HTML page with inline Clojure interpreted by Scittle 0.8.33; Chromium verified default, Unicode, blank and literal HTML greetings, button and Enter submission. No application build step. |
| Jolt / tail-call Fibonacci | [Native demo + comparison output](examples/jolt/) | Linux ARM64, Chez 10.4.1: ordinary tail recursion computes Fibonacci(20000), checked against an independent reference; identical function overflows on JVM Clojure with a controlled 256 KiB stack. No Docker required. |
...
| Swish | SwiftUI screen whose greeting is computed by an embedded Swish function called from Swift. | A native Swift application with an embedded scripting layer. | Reload the greeting script without rebuilding the host app. |
...
| Jolt | Compare naive Fibonacci with an accumulator using ordinary tail calls, then contrast with JVM stack overflow. | Proper tail calls inherited from Chez Scheme, without loop/recur. | Run a self-interpreter tower on the same host. |
