...
| ClojureDart + Flutter | [Demo + screenshots](examples/clojuredart/) | Web release, native Linux ARM64 + GTK (host build/run, no Docker), and iOS 26.5 simulator: Unicode/blank greetings and counter integration checks passed. Newest-first scrollable history additionally checked on Linux and web. |
...
| Clojurust / immutable history | [REPL + AOT eval demo](examples/clojurust/) | Native Linux ARM64: persistent snapshots, atom updates, undo, duplicate/Unicode greetings, and live function redefinition verified; relocated AOT executable evaluates argument expressions with matching interpreted output (documented eval workaround). |
| Rustly / Raspberry Pi 1 + 4 firmware | [Hardware hello + Morse demo](examples/rustly-rpi/) | Original Clojure-to-Rust hello passed physical Pi 4 boot and Unicode echo. Serial-controlled Morse extension passes sequencer and QEMU tests; Morse image passed physical Pi 4 boot and accepted `clj` over serial; visual LED verification pending. Original Pi 1 A/B port passes build, QEMU serial tests, physical boot and `clj` serial acknowledgement; user confirmed the Pi 1 ACT LED repeats `clj` in Morse. |
